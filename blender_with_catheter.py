...
def import_sequence():
    """Import regular contour OBJ files (mesh_XXX_rest.obj) and return groups of imported objects."""
    imported_groups = []
    for i in range(32):
...
def import_catheter_sequence():
    """Import catheter contour OBJ files (catheter_XXX_rest.obj) and return groups of imported objects."""
...
def setup_animation(groups):
    ...
    for obj_idx, group in enumerate(groups):
        # Calculate visible frames. (Customize this as needed.)
        if obj_idx == 0:
            frames = [1]
        elif obj_idx == 31:
            frames = [32]
    ...
    # print the order of the objects
    for obj in groups:
        print("PRINTING OBJ", obj[0].name)
...
if __name__ == "__main__":
    clean_scene()
    # Import both contour and catheter meshes
    groups = import_sequence()
    catheter_groups = import_catheter_sequence()
    
    # Setup animation for each group set
    setup_animation(groups)
    setup_animation(catheter_groups)
