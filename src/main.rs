...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if config.processing.run_process_case {
        process_case("rest", &config.general.rest_input_path, &config.general.rest_output_path)?;
        process_case("stress", &config.general.stress_input_path, &config.general.stress_output_path)?;
    }
